...
function getBases(JSONInput) {
	...
	for (let i = 0; i < bases.length; i++) {
		...
		if (base.BaseType.PersistentBaseTypes == 'FreighterBase') baseContainer.insertAdjacentHTML('afterbegin', buildImmovable('Freighterbase'));
		const element = buildElement(id, name);
		baseContainer.insertAdjacentHTML('beforeend', element);
	}
	document.querySelectorAll('#checkboxes input').forEach(e => {
		extraItem(e);
	})
	const sortable = new Sortable(baseContainer, {
		animation: 250,
		filter: '.immovable',
	});
}
